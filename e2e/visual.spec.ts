...
// appends the changelog entry in the "v$V" commit AFTER this build's snapshots
// are captured (see scripts/fixloop.sh run_proposal), so the badge - and, on
// the pipeline panel, the "latest vX" run card that reads the same changelog -
// change on every single drop. Pixel-asserting them means the baseline is stale
// the moment it is committed, which is exactly what kept re-breaking these
// snapshots. Mask the live release stamp so the snapshots assert the stable
// layout, not the version of the day.
...
test("pipeline panel looks right @visual", async ({ page }) => {
  ...
  // clamped to 88 chars - one to three lines depending on its length. When a
  // drop changes that headline the card's height changes, reflowing every row
  // below it (that height cascade, not any real regression, is the ~8k-pixel
  // diff that kept tripping this snapshot). Pin the blurb to a single line so
  // the card footprint is constant, then mask the card so its live text is
  // ignored - together they make the panel layout below it deterministic.
